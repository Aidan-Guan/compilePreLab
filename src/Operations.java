...
public class Operations {
    ...
    public static void readLine(String line) {
        ...
        for (String token : tokens) {
            ...
            // 判断是否为无符号整数
            if (isDigit(token)) { System.out.println("Number("+token+")"); }
            // 判断是否为标识符
            else if (isIdentifier(token) && !speList.contains(token)) { System.out.println("Ident("+token+")"); }
            else {
                // 判断是否为其他的东西
                switch (token) {
        ...
        for (String str : priTokens) {
            ...
            for (; index < str.length() ; index += 1) {
                ...
                // 出现了字符但是不是等号
                    ...
                    // 出现了数字开头接字母
                    if (tmp.length()>=1 && Character.isDigit(tmp.charAt(0)) && !Character.isDigit(str.charAt(index))) {
                        tokens.add(tmp);
                        tmp = "";
                    }
